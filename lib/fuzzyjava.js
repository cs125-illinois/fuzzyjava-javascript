...
class FuzzyJava extends Visitor {
  constructor (string) {
    ...
    this.errorListener = new ErrorListener(this.errors)
    this.state.parser.removeErrorListeners()
    this.state.parser.addErrorListener(this.errorListener)
...
ErrorListener.prototype.syntaxError = function(recognizer, symbol, line, column, msg, e) {
  if (e) {
    this.errors.push(`Syntax error on ${line}:${column}`)
  }
}
