...
class FuzzyJava extends Listener {
  ...
  enterVariableDeclaratorId(ctx) {
    ...
    if (name.startsWith('__') && name.endsWith('__')) {
      name = ctx.text = camelcase(randomWords(_.sample([1, 2])).join('-'))
    }
  ...
  exitLiteral(ctx) {
    ...
    if (literal === '__random__') {
      switch (this.variable.type) {
        case 'byte':
          ctx.text = randomInt(-128, 127)
          break
        case 'short':
          ctx.text = randomInt(-32768, 32767)
          break
        case 'int':
          ctx.text = randomInt(-Math.pow(2, 31), Math.pow(2, 31) - 1)
          break
        case 'long':
          ctx.text = randomInt(-Math.pow(2, 63), Math.pow(2, 63) - 1)
          break
        case 'float':
        case 'double':
          ctx.text = randomFloat(-32768, 32767)
          break
        case 'char':
          ctx.text = `'${randomUnicode(1)}'`
          break
      }
  ...
  exitVariableDeclarator(ctx) {
    this.variables[this.variable.name] = this.variable
    this.variable = {}
