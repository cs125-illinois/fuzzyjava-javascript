...
const globalConfig = yamljs.load(path.join(__dirname, './fuzzyjava.yaml'))
...
class FuzzyJava extends Visitor {
  ...
  visitPrimitiveType (ctx) {
    ...
    if (type === '?primitive') {
      ...
      type = ctx.text = _.sample(['short', 'int', 'long', 'float', 'double'])
    }
    this.currentVariable = {
      type: type
    }
  ...
  visitVariableDeclaratorId (ctx) {
    ...
    if (originalName.startsWith('?')) {
      ...
      if (!newName) {
        ...
        }) || _.find(globalConfig.reserved, reserved => {
